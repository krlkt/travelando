...
import { cityOverrideDraftSchema } from '@/lib/trips/schemas';
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const overrides = await repo.listCityOverrides(tripId);
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const parsed = cityOverrideDraftSchema.safeParse({
    ...(body as object),
    tripId,
  });
  if (!parsed.success) {
  ...
  try {
    const repo = createSupabaseRepository(supabase);
    const override = await repo.upsertCityOverride(parsed.data);
    return NextResponse.json(
