...
import { dayPlanDraftSchema } from '@/lib/trips/schemas';
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const plans = await repo.listDayPlans(tripId);
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const parsed = dayPlanDraftSchema.safeParse({
    ...(body as object),
    tripId,
  });
  if (!parsed.success) {
  ...
  try {
    const repo = createSupabaseRepository(supabase);
    const plan = await repo.upsertDayPlan(parsed.data);
