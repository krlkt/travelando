...
import { foodPlaceDraftSchema } from '@/lib/trips/schemas';
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const places = await repo.listFoodPlaces(tripId);
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const parsed = foodPlaceDraftSchema.safeParse({
    ...(body as object),
    tripId,
  });
  if (!parsed.success) {
  ...
  try {
    const repo = createSupabaseRepository(supabase);
    const place = await repo.addFoodPlace(parsed.data);
